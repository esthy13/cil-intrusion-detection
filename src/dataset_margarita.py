...
def load_csv_folder_from_zip(zip_path, folder):
    dfs = []
    ...
    with zipfile.ZipFile(zip_path, 'r') as z:
        ...
        if not csv_files:
            raise ValueError(f"No CSV files found in {folder}/ inside {zip_path}")
...
class CILDataset:
    def __init__(self, path_zip, dataset_name, split, label_col, feature_cols):
